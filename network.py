...
class TNet(nn.Module):
    def __init__(self, available_actions_count, mem):
        ...
        self.lstm_size = 512
        self.lstm = nn.LSTM(6400, self.lstm_size, batch_first=True)
        self.mem = mem
        ...
        self.h0b = Variable(torch.zeros(1, 1, self.lstm_size), requires_grad=True).cuda()
        self.c0b = Variable(torch.zeros(1, 1, self.lstm_size), requires_grad=True).cuda()
    ...
    def forward(self, x):
        ...
        if self.mem:
            ...
            self.acts = self.action_value_layer(x)
            return self.acts
        else:
            batch_size = x.size()[0]
